(ns dataseq-core.events
  "Provides a very simply event bus using `core.async` to allow publishing and subscribing to interesting topics
  happening throughout the dataseq-core system in a decoupled way.

  ## Regarding Events Initialization:

  The most appropriate way to initialize event listeners in any `dataseq-core.events.*` namespace is to implement the
...
(defn- find-and-load-event-handlers!
  "Search Classpath for namespaces that start with `dataseq-core.events.`, and call their `events-init` function if it
  exists."
  []
  (log/info "is-test?: %s" config/is-test?)
  (when-not config/is-test?
    (doseq [ns-symb @u/dataseq-core-namespace-symbols
            :when   (.startsWith (name ns-symb) "dataseq-core.events.")]
