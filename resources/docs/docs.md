...
The `home-routes` handler in the `dataseq-core.routes.home` namespace
...
The `home-page` function will in turn call the `dataseq-core.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `dataseq-core.handler` namespace:
...
Request middleware functions are located under the `dataseq-core.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `dataseq-core.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `dataseq-core.core` in some other namespace.
